###
#
# Lenovo Redfish examples - Mount virtual media from remote disc on card (RDOC)
#
# Copyright Notice:
#
        ...
        if response_managers_url.status == 200:
            ...
            for i in range(count):
                ...
                if response_remotecontrol_url.status == 200:
                    ...
                    # Build request body and send requests to upload media iso
                    protocol = fsprotocol.lower()
                    if protocol == "nfs":
                        source_url = "nfs://" + fsip + '/' + fsdir + "/" + isoname
                    elif protocol == "samba":
                        source_url = "smb://" + fsip + '/' + fsdir + "/" + isoname
                    elif protocol in ['sftp', 'ftp', 'http', 'https']:
                        source_url = protocol + "://" + fsip + '/' + fsdir + "/" + isoname
                    else:
                        result = {'ret': False,
                                  'msg': 'Mount media iso RDOC only support protocol Samba, NFS, HTTP/HTTPS, SFTP/FTP'}
                        return result

                    body = {"sourceURL":source_url, "Username":fsusername, "Password":fspassword, "Type":fsprotocol, "Readonly":bool(readonly), "Domain":domain, "Options":options}
                    ...
                    if response_upload_url.status == 200:
                        ...
                        result = {'ret': False, 'msg': "Url '%s' response Error code %s \nerror_message: %s" % (upload_url, response_upload_url.status, error_message)}
                        return result
                ...
                if response_remotemap_url.status == 200:
                    ...
                    if response_mount_image.status == 200:
                        result = {'ret': True, 'msg': "operation runs successfull,all the mounted image resources under MountImage will be changed to mounted state"}
                    else:
...
def add_helpmessage(argget):
    ...
    argget.add_argument('--fsprotocol', type=str, nargs='?',
    ...
    argget.add_argument('--fsusername', type=str, nargs='?',
                        help='Username to access the file path, available for Samba, NFS, HTTP/HTTPS, SFTP/FTP')
    argget.add_argument('--fspassword', type=str, nargs='?',
                        help='Password to access the file path, password should be encrypted after object creation, available for Samba, NFS, HTTP/HTTPS, SFTP/FTP')
...
def add_parameter():
    ...
    argget = argparse.ArgumentParser(
        description="Remote disc on card RDOC, (50 MB available)")
