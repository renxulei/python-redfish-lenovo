###
#
# Lenovo Redfish examples - get manager LDAP information
#
# Copyright Notice:
#
...
def get_ldap_info(ip, login_account, login_password):
        ...
        if response_managers_url.status == 200:
            for request in response_managers_url.dict['Members']:
                ...
                if response_ldap_client.status == 200:
                    for key in response_ldap_client.dict:
                        if key not in ["Description","@odata.etag","@odata.id","@odata.type","@odata.context"]:
                            ldap_client_info[key] = response_ldap_client.dict[key]
...
if __name__ == '__main__':
    ...
    result = get_ldap_info(ip, login_account, login_password)
